# LLM Inferencing on AKS Automatic with Ray Serve

Deploy a complete LLM serving stack on [AKS Automatic](https://learn.microsoft.com/azure/aks/intro-aks-automatic) using [KubeRay](https://github.com/ray-project/kuberay) and Ray Serve with vLLM, exposing an OpenAI-compatible REST API.
...
1. An Azure subscription. If you don't have one, create a free account [here](https://azure.microsoft.com/free/).
2. The [Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli).
3. [kubelogin](https://github.com/Azure/kubelogin) for Azure RBAC authentication.
...
2. Update the configuration in `setup-aks-rayserve-llm.sh`:
   ```bash
...
Edit `ray-service.llm-serve.yaml` to use a different model:
